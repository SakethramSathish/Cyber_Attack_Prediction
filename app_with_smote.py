...
from sklearn.model_selection import train_test_split  #This is used to split the dataset into testing and training data
from sklearn.preprocessing import StandardScaler, MinMaxScaler  #This will be used for normalizing the dataset so that the model doesn't go wrong in any case. Like making all the values in one range which is usually from 0 to 1
from sklearn.ensemble import RandomForestClassifier #This will be used to train the model using multiple decision trees
from sklearn.metrics import accuracy_score, classification_report, confusion_matrix, roc_auc_score  #These are built-in functions to caluculate the performance metrics after the training is done
from sklearn.utils import class_weight  #This is used to address the class imbalance since there is more normal classes than attacks, so the model is punished more if it wrongly classifies an attack and the model is punished less if it wrongly classifies a normal
from sklearn.base import BaseEstimator, ClassifierMixin #This is used to create custom ML models using scikit-learn
import tensorflow as tf #This whole package is used for the whole DL part
from tensorflow import keras  #Keras makes handling TF easier.
from tensorflow.keras import layers #This is the function that provides the layers to construct the neural networks
from tensorflow.keras.callbacks import EarlyStopping  #This function immediately stops training if it sees overfitting take place
from tensorflow.keras.models import Sequential  # Used to create a simple stack of layers for the neural network
from tensorflow.keras.layers import Dense, Dropout, BatchNormalization  # Dense: fully connected layer; Dropout: avoids overfitting; BatchNormalization: stabilizes training
from tensorflow.keras.optimizers import Adam  # Optimizer that adjusts learning rate automatically for efficient training
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau  # EarlyStopping halts training when no improvement; ReduceLROnPlateau lowers learning rate when progress slows

from sklearn.neighbors import NearestNeighbors  # Finds the nearest data points (used for anomaly detection or neighborhood-based models)
from sklearn.metrics import roc_curve, auc  # Used to plot ROC curve and calculate AUC for model performance
from scipy.spatial.distance import cdist  # Computes pairwise distances between samples (useful for clustering or GAN evaluation)
import scipy.stats as stats  # Provides statistical functions (mean, variance, hypothesis testing, etc.)
from sklearn.manifold import TSNE  # Reduces high-dimensional data to 2D/3D for visualization (like visualizing attack clusters)
import plotly.graph_objects as go  # Used to create interactive and dynamic plots for visualizing data
from plotly.subplots import make_subplots  # Helps organize multiple Plotly plots into one combined layout

from imblearn.combine import SMOTETomek  # Combines over-sampling (SMOTE) and under-sampling (Tomek Links) to balance dataset
from imblearn.under_sampling import TomekLinks  # Removes ambiguous samples between classes to clean the dataset
from imblearn.over_sampling import SMOTE, ADASYN, BorderlineSMOTE  # Over-sampling techniques that generate synthetic minority samples
from imblearn.combine import SMOTETomek, SMOTEENN  # Hybrid sampling methods combining both over- and under-sampling strategies

from xgboost import XGBClassifier  # Powerful gradient boosting ML algorithm for classification (often more accurate than Random Forest)
from lightgbm import LGBMClassifier  # Lightweight, fast gradient boosting algorithm optimized for large datasets
from sklearn.model_selection import cross_val_predict, KFold  # Cross-validation tools to evaluate model performance across multiple splits
